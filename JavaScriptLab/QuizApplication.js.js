...
const q5 = new Question("JavaScript is a ____", 5);

function Answer(answerText){
  this.answerText = answerText
}

function QaPair (questionObj, multipleAnsObj, correctAnswerObj) {

  this.questionObj = questionObj;
  this.multipleOptionsObj = multipleAnsObj;
  this.correctAnswerObj = correctAnswerObj;

  this.checkAnswer = function(userAnswer){

    if (correctAnswerObj.answerText === userAnswer){
...
const qaPair1 = new QaPair(
  q1, [aFunctions, new Answer("XHTML"),
    aCSS, aHTML ],  aFunctions);

const aJQuery = new Answer("JQuery")
const qaPair2 = new QaPair(
  q2, [ aHTML, aJQuery, aCSS, new Answer("XML")], aCSS);

const aPythonScript = new Answer("Python Script");
const qaPair3 = new QaPair(
  q3, [aPythonScript, aJQuery,new Answer("Django"), new Answer("Node JS")],aPythonScript);

const aPHP = new Answer("PHP");
const aAll = new Answer("All");
const qaPair4 = new QaPair(
  q4, [aPHP, aHTML, new Answer("JS"), aAll],aPHP);

const aProgrammingLanguage = new Answer("Programming Language");
const qaPair5 = new QaPair(
  q5, [new Answer("Language"), aProgrammingLanguage,new Answer("Development"), aAll], aProgrammingLanguage);
...
function quizCompetition (qaPairArray) {
  ...
  this.associateEventListener = function(
    ...
    buttonElement.onclick = function(event){
      currentQuizAppObject.AnswerSelection(event);
    }  
  }

  this.AnswerSelection = function(event){
    const target = event.currentTarget;
    const userAnswer = target.children[0].innerText;
    const qaPair = qaPairArray[this.pageIndex];

    const outcome = qaPair.checkAnswer(userAnswer);
    if (outcome){
      this.incrementScore();
    }
    this.nextPage();
...
const quizApp = new quizCompetition(
